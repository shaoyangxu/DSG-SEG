...
def read_clinical_file(args, filtered_num, all_num, filtered_document_num, all_document_num, path, encoder=None,length_filter=10, sent_num_filter=10):
    seperator = '=========='
        ...
        if sentences:
            ...
            for sentence in sentences:
                ...
                if args.sr_choose == 'g_model' and args.gr_choose == 'texting':
                    words, adj = build_ing_graph(words, args)
                    adjs.append(adj)
                if encoder:
...
class ClinicalDataSet(Dataset):
    def __init__(self,
        ...
        all_document_num = 0
        rank_logger_info(logger, local_rank, "Reading clinical data")
        for textfile in self.textfiles:
            return_dict, filtered_num, all_num, filtered_document_num, all_document_num = read_clinical_file(args,filtered_num,
                                                                                                        all_num,
...
if __name__ == "__main__":
    ...
    clinicaldataset = ClinicalDataSet(args.clinical_path, args)